...
from .bots_api_utils import BotCreationSource, create_bot, create_bot_chat_message_request, create_bot_media_request_for_image, delete_bot, patch_bot, patch_bot_presence_indicator, patch_bot_transcription_settings, patch_bot_voice_agent_settings, send_sync_command
...
class PresenceIndicatorView(APIView):
    ...
    @extend_schema(
        operation_id="Update Presence Indicator",
        summary="Update the bot's presence indicator",
        description="Sets the mark the bot draws over its own avatar: 'listening' pulses slowly, 'working' pulses faster, 'speaking' and 'off' show nothing. The bot animates the video it is already sending, so this costs one call per change rather than one per frame.",
        request=PatchBotPresenceIndicatorSerializer,
        responses={
            200: OpenApiResponse(description="Presence indicator updated successfully"),
            400: OpenApiResponse(description="Invalid input"),
            404: OpenApiResponse(description="Bot not found"),
        },
        parameters=[
    ...
    def patch(self, request, object_id):
        ...
        bot_updated, error = patch_bot_presence_indicator(bot, request.data)
        if error:
            return Response(error, status=status.HTTP_400_BAD_REQUEST)

        try:
            send_sync_command(bot, "sync_presence_indicator")
            return Response(status=status.HTTP_200_OK)
        except Exception as e:
            logging.error(f"Error syncing presence indicator for bot {bot.object_id}: {str(e)}")
            return Response({"error": "Failed to update the presence indicator"}, status=status.HTTP_400_BAD_REQUEST)
