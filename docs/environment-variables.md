...
| `WEBPAGE_STREAMER_IS_SHARED` | Boolean | `false` | The webpage streamer is one always-on service shared by every bot, rather than one spawned per bot. Set it on both the worker and the streamer itself. A bot leaving no longer shuts the streamer down, and the streamer no longer exits when no keepalive has arrived for 15 minutes: it releases the idle session — browser, capture pipeline, peer connections — and keeps serving, building a fresh one on the next request. Leave it unset for the per-bot deployment, where the idle timeout must end the process so its Chrome goes with it. |
